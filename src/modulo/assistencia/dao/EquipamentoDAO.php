...
class EquipamentoDAO {
    ...
    public function inserir($equipamento) {
        try {
            $query = "INSERT INTO equipamento (nr_serie,crcliente_id,id_modelo,teste_ok,inativo) "
                    . "VALUES (UPPER(?),?,?,?,?)";
            ...
            $stmt->bindValue(4, $equipamento->getTesteOK());
            $stmt->bindValue(5, $equipamento->getInativo());
            $stmt->execute();
    ...
    public function atualizar($equipamento) {
        try {
            $query = "UPDATE equipamento SET nr_serie=UPPER(?),crcliente_id=?,id_modelo=?,teste_ok=?,inativo=? "
            ...
            $stmt->bindValue(4, $equipamento->getTesteOK());
            $stmt->bindValue(5, $equipamento->getInativo());
            $stmt->bindValue(6, $equipamento->getCodigo());
            $stmt->execute();
        } catch (PDOException $e) {
            if ($e->errorInfo[0] === '23505') {
                $motivo = 'O equipamento (' . $equipamento->getNrSerie() . ') já está cadastrado e não pode ser duplicado. Tente fazer uma busca.';
    ...
    public function listar($busca, $offset, $rows, $sort, $order) {
        try {
            $query = "SELECT * FROM vw_equipamento "
                    . "WHERE LOWER(nr_serie) LIKE LOWER(:busca) "
    ...
    public function listarEquipamento($id) {
        try {
            $query = "SELECT * "
                    . "FROM vw_equipamento "
                    . "WHERE codigo_equipamento = ?";
